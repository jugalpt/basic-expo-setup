...
function ListItem({ item, checkItemExists, addItem }) {
  console.log(`${item.colorName} rendering list item`);
  const isItemExists = checkItemExists(item.colorName);
  return (
    <View style={styles.listItem}>
      <Text>{item.colorName}</Text>
      <Switch onValueChange={() => addItem({ ...item })} value={isItemExists} />
    </View>
  );
}
...
const addItemsToPallete = (currentColorName, currentHexCode, prev) => {
  const isItemExists = prev.find(
    ({ colorName }) => colorName === currentColorName,
  );

  if (isItemExists) {
    return prev.filter(({ colorName }) => colorName !== currentColorName);
  }

  return [{ colorName: currentColorName, hexCode: currentHexCode }, ...prev];
};
...
export default function ColorPaletteModal({ navigation }) {
  const { status, setState: setGlobalState } = useData();
  ...
  const handleText = useCallback(
    arg => setState(prev => ({ ...prev, paletteName: arg })),
    [],
  );

  const checkItemExists = useCallback(
    currentColorName =>
      !!state.colors.find(({ colorName }) => colorName === currentColorName),
    [state.colors],
  );

  const addItem = useCallback(({ colorName, hexCode }) => {
    setState(prev => ({
      ...prev,
      colors: addItemsToPallete(colorName, hexCode, prev.colors),
    }));
  }, []);
  ...
  const handleSubmit = () => {
    if (!state.paletteName) {
      console.log('fired');
      createTwoButtonAlert('Title Error', 'palette name is required!');
      return;
    }
  ...
  return (
    <View style={styles.container}>
      <Text>Name of your color Pallete</Text>
      <TextInput
      ...
      <View style={{ flex: 1, height: '100%' }}>
        ...
        />
        <Button style={{ padding: 16 }} title="Submit" onPress={handleSubmit} />
      </View>
    </View>
  );
}
